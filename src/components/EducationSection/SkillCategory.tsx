...
export default function ProfessionalProfile() {
  const technicalSkills = [
    { name: "C & C++ Programming", level: 90, icon: Code2, color: "text-cyan-400" },
    { name: "Python", level: 85, icon: Atom, color: "text-emerald-400" },
    { name: "JavaScript & Web (HTML/CSS/Node.js)", level: 88, icon: Server, color: "text-blue-400" },
    { name: "Databases (MySQL / MongoDB)", level: 82, icon: Database, color: "text-amber-400" },
    { name: "Hardware, OS & IoT (Arduino/Sensors)", level: 92, icon: Cloud, color: "text-purple-400" },
  ];

  const softSkills = [
    { name: "Problem Solving", icon: Brain, color: "text-purple-400 border-purple-500/30 bg-purple-500/10" },
    { name: "Analytical Thinking", icon: Lightbulb, color: "text-yellow-400 border-yellow-500/30 bg-yellow-500/10" },
    { name: "Communication", icon: Workflow, color: "text-sky-400 border-sky-500/30 bg-sky-500/10" },
    { name: "Teamwork", icon: Users, color: "text-emerald-400 border-emerald-500/30 bg-emerald-500/10" },
    { name: "Leadership", icon: Crown, color: "text-amber-400 border-amber-500/30 bg-amber-500/10" },
    { name: "Time Management", icon: HeartHandshake, color: "text-rose-400 border-rose-500/30 bg-rose-500/10" },
    { name: "Quick Learning", icon: Rocket, color: "text-cyan-400 border-cyan-500/30 bg-cyan-500/10" },
  ];
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-2">
        ...
        <h3 className="text-2xl md:text-3xl font-extrabold tracking-tight">Expertise & Capabilities</h3>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="glass-panel p-8 rounded-[2rem] border border-foreground/15 shadow-xl relative overflow-hidden">
          ...
          <div className="mt-8 pt-6 border-t border-border/60">
            ...
            <div className="flex flex-wrap gap-2">
              {["VS Code", "Git", "GitHub", "XAMPP", "MS Office", "Linux / OS"].map((tool, idx) => (
                <span key={idx} className="px-3 py-1 rounded-lg bg-foreground/5 border border-foreground/10 text-xs font-semibold text-foreground">
                  {tool}
                </span>
              ))}
            </div>
        ...
        <div className="glass-panel p-8 rounded-[2rem] border border-foreground/15 shadow-xl flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-8 pb-4 border-b border-border/60">
              <h4 className="text-xl font-bold text-foreground flex items-center gap-2">
                <Brain className="w-5 h-5 text-primary" /> Soft Skills & Attributes
              </h4>
              <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground bg-muted/60 px-3 py-1 rounded-full border border-border/50">
                Core Strengths
              </span>
            ...
            <div className="mt-8 pt-6 border-t border-border/60 space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <span className="px-3 py-1 rounded-xl bg-primary/10 border border-primary/20 text-xs font-bold text-primary">
                    Tamil (Native)
                  </span>
                  <span className="px-3 py-1 rounded-xl bg-primary/10 border border-primary/20 text-xs font-bold text-primary">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  {["Web Development", "IoT & Embedded Systems", "Artificial Intelligence", "Open Source Technologies"].map((item, idx) => (
                    <span key={idx} className="px-2.5 py-1 rounded-lg bg-foreground/5 border border-foreground/10 text-foreground font-medium">
                      {item}
                    </span>
                  ))}
                </div>
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-r from-primary/10 via-purple-500/5 to-transparent border border-primary/20 flex items-start gap-3.5 shadow-sm">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Driven to contribute effectively to innovative technology organizations while expanding proficiency across emerging web, AI, and smart computing frameworks.
                </p>
              </div>
            </div>
          </div>
